...
import { GAMES_COUNT, Player, Position, Score, ScoreLayout, PlayerLayout, TOTAL_PLAYERS_COUNT } from '../../../sdk/state';
...
export const InstructionsTests = () =>
  ...
  describe('Create createInitializeRootInstruction', () => {
    ...
    // console.log(a);
    // throw a;
    const players =  Array.from({ length: TOTAL_PLAYERS_COUNT })
      .map((): Player => ({
        id: 1,
        position: Position.DB,
        scores: Array.from({ length: GAMES_COUNT })
        .map((): Score => ({
          score1: 1,
          isInitialized: true
        })),
