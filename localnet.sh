...
up)
  ...
  (
    ...
    docker run "${ARGS[@]}" solanalabs/solana:"$channel" || docker start solana-localnet1
...
down)
  (
    set -x
    if [[ $(docker ps --filter "name=^/solana-localnet1$" -q) ]]; then
      docker stop --time 0 solana-localnet1
    fi
  )
  ;;
logs)
  ...
  while $follow; do
    if [[ $(docker ps -q -f "name=^/solana-localnet1$") ]]; then
      (
        set -x
        docker logs solana-localnet1 -f
      ) || true
    fi
    sleep 1
  done
