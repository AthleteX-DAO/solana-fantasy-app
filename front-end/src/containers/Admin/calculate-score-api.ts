...
export async function calculateScore(
  ...
  week: number,
  pushLine: (line: { text: string; color: string }) => void,
  popLine: () => void
) {
  console.log('number of players for calculating score', playerExternalIds.length);
  pushLine({
    text: `Number of players for calculating score: ${playerExternalIds.length}`,
    color: 'white',
  });

  pushLine({
    text: `Making API call to https://api.sportsdata.io`,
    color: 'white',
  });
  ...
  pushLine({
    text: `Received player stats from API only ${playerGameStatsArr.length} players.`,
    color: 'white',
  });
  ...
  await wait(500);
  pushLine({
    text: `Using ESPN rules to calculate score of each player...`,
    color: 'white',
  });
  ...
  pushLine({
    text: `Progress: 0/${playerGameStatsArr.length}`,
    color: 'white',
  });
  for (const [i, playerGameStats] of playerGameStatsArr.entries()) {
    await wait(7);
    popLine();
    pushLine({
      text: `Progress: ${i + 1}/${playerGameStatsArr.length}`,
      color: 'white',
    });
