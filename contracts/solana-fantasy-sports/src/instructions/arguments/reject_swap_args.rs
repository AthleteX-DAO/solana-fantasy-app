...
#[repr(C)]
pub struct RejectSwapArgs<'a> {
    data: &'a RefCell<&'a [u8]>,
    offset: usize,
}
impl<'a> RejectSwapArgs<'a> {
    ...
    fn slice<'b>(
        ...
        array_refs![
            array_ref![data, self.offset, RejectSwapArgs::LEN],
            2,
            1,
            1,
            2,
    ...
    pub fn copy_to(&self, to: &mut [u8]) {
        ...
        array_mut_ref![to, self.offset, RejectSwapArgs::LEN].copy_from_slice(array_ref![
            src,
            self.offset,
            RejectSwapArgs::LEN
        ]);
    ...
    pub fn new(data: &'a RefCell<&'a [u8]>, offset: usize) -> Result<RejectSwapArgs, ProgramError> {
        if data.borrow().len() < Self::LEN + offset {
            return Err(ProgramError::InvalidInstructionData);
        }
        Ok(RejectSwapArgs { data, offset })
    }
}
impl Clone for RejectSwapArgs<'_> {
    fn clone(&self) -> Self {
        Self {
            data: self.data,
            offset: self.offset,
