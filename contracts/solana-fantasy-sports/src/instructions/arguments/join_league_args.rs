...
impl<'a> JoinLeagueArgs<'a> {
    ...
    pub fn copy_to(&self, to: &mut [u8]) {
        ...
        array_mut_ref![to, self.offset, JoinLeagueArgs::LEN].copy_from_slice(array_ref![
            src,
            self.offset,
            JoinLeagueArgs::LEN
        ]);
    ...
    pub fn new(data: &'a RefCell<&'a [u8]>, offset: usize) -> Result<JoinLeagueArgs, ProgramError> {
        if data.borrow().len() < Self::LEN + offset {
            return Err(ProgramError::InvalidInstructionData);
        }
        Ok(JoinLeagueArgs { data, offset })
    }
}
impl Clone for JoinLeagueArgs<'_> {
    fn clone(&self) -> Self {
        Self {
            data: self.data,
            offset: self.offset,
